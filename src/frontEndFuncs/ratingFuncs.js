...
async function getRating(rating_id) {
    const rating_url = new URL('rating/' + rating_id, process.env.REACT_APP_BACKEND_URL)
...
async function getRatinglist(client_id) {
    const rating_url = new URL(`ratings/${client_id}`, process.env.REACT_APP_BACKEND_URL)
    return fetch(rating_url, {
        method: "GET",
        headers: {
            "Content-Type": "application/json"
